...
const unused = pool.filter(t => !usedTheory.has(t.title));
...
const unused = pool.filter(p => !usedPractice.has(p.title));
...
if (repeatable.length > 0) return repeatable[dayNum % repeatable.length];
return null;
}
...
equipment: { bg: “linear-gradient(135deg, #E8EAF6, #E3F2FD)”, border: “#7986CB”, tag: “#283593”, tagBg: “#E8EAF6”, label: “Equipment” }
};
...
<div onClick={e => e.stopPropagation()} style={{ background: “#FFFDF7”, borderRadius: 24, maxWidth: 620, width: “100%”, maxHeight: “85vh”, overflow: “auto”, padding: “28px 24px”, boxShadow: “0 25px 60px rgba(0,0,0,0.3)”, animation: “slideUp 0.4s ease” }}>
...
{lesson.tips.map((tip, i) => (<div key={i} style={{ display: “flex”, gap: 8, marginBottom: 6, padding: “8px 12px”, background: “#FFF8E1”, borderRadius: 10, fontSize: 13, color: “#4E342E”, fontFamily: “‘DM Sans’, sans-serif” }}><span style={{ color: “#FFB74D”, fontWeight: 700 }}>✦</span><span>{tip}</span></div>))}</div>)}
...
{lesson.proTip && (<div style={{ background: “linear-gradient(135deg, #FFF3E0, #FFECB3)”, borderRadius: 14, padding: “14px 16px”, border: “2px dashed #FFB74D”, marginBottom: 12 }}><div style={{ fontFamily: “‘Space Mono’, monospace”, fontSize: 11, fontWeight: 700, color: “#E65100”, marginBottom: 4 }}>🤙 PRO-TIPP</div><p style={{ fontFamily: “‘DM Sans’, sans-serif”, fontSize: 14, color: “#4E342E”, margin: 0, fontWeight: 500 }}>{lesson.proTip}</p></div>)}
{lesson.keyTerms && (<div style={{ marginTop: 12 }}><div style={{ display: “flex”, flexWrap: “wrap”, gap: 5 }}>{lesson.keyTerms.map((t, i) => <span key={i} style={{ background: “#ECEFF1”, color: “#546E7A”, padding: “3px 10px”, borderRadius: 20, fontSize: 11, fontFamily: “‘Space Mono’, monospace” }}>{t}</span>)}</div></div>)}
</div>
</div>
);
};
...
const [program, setProgram] = useState(hasSaved ? generateProgram(saved.days, saved.goal, saved.spot, saved.equipment) : null);
...
const build = () => { if (!days || !goal || !spot) return; const eq = { board: board || “none”, experience: experience || “zero” }; const p = generateProgram(days, goal, spot, eq); setProgram(p); setScreen(“program”); setActiveDay(null); setCompleted({}); saveData({ days, goal, spot, board, experience, equipment: eq, completed: {} }); };
const continueSaved = () => { if (!hasSaved) return; setDays(saved.days); setGoal(saved.goal); setSpot(saved.spot); setBoard(saved.board || “”); setExperience(saved.experience || “”); setProgram(generateProgram(saved.days, saved.goal, saved.spot, saved.equipment)); setCompleted(saved.completed || {}); setScreen(“program”); setActiveDay(null); };
...
const savedTotal = hasSaved ? generateProgram(saved.days, saved.goal, saved.spot, saved.equipment).program.reduce((s, d) => s + d.lessons.length, 0) : 0;

return (
<>
<style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700;800;900&family=Space+Mono:wght@400;700&family=DM+Sans:wght@400;500;600;700&display=swap'); @keyframes slideUp { to { opacity: 1; transform: translateY(0); } } @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } } @keyframes float { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-10px); } } @keyframes wave { 0% { transform: rotate(0deg); } 25% { transform: rotate(20deg); } 75% { transform: rotate(-15deg); } 100% { transform: rotate(0deg); } } * { box-sizing: border-box; margin: 0; padding: 0; } input:focus { outline: 2px solid #FFB74D; outline-offset: 2px; } input[type="range"] { -webkit-appearance: none; width: 100%; height: 8px; background: linear-gradient(90deg, #4DB6AC, #009688); border-radius: 4px; outline: none; } input[type="range"]::-webkit-slider-thumb { -webkit-appearance: none; width: 28px; height: 28px; border-radius: 50%; background: white; border: 3px solid #009688; cursor: pointer; box-shadow: 0 2px 8px rgba(0,0,0,0.2); } input[type="range"]::-moz-range-thumb { width: 28px; height: 28px; border-radius: 50%; background: white; border: 3px solid #009688; cursor: pointer; } ::-webkit-scrollbar { width: 6px; } ::-webkit-scrollbar-thumb { background: #CFD8DC; border-radius: 3px; }`}</style>
...
<div><h1 style={{ fontFamily: “‘Playfair Display’, serif”, fontSize: 22, fontWeight: 900, color: “#263238”, lineHeight: 1 }}>Soul<span style={{ color: “#009688” }}>Surf</span></h1>
<span style={{ fontFamily: “‘Space Mono’, monospace”, fontSize: 9, color: “#78909C”, letterSpacing: “0.15em”, textTransform: “uppercase” }}>ride the vibe ☮</span></div>
</div>
...
```
      {screen === "home" && (
        <div style={{ paddingTop: 50, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "#546E7A", maxWidth: 440, margin: "0 auto 40px", lineHeight: 1.6 }}>Dein persönliches Surf-Programm – angepasst an dein Level, Ziel und deinen Spot. 3 bis 30 Tage, mit Equipment-Guide, Videos und Artikeln.</p>
          {hasSaved && (
            <div style={{ background: "linear-gradient(135deg, #004D40, #00695C)", borderRadius: 20, padding: "24px 20px", marginBottom: 24, textAlign: "left", color: "white", position: "relative", overflow: "hidden", animation: "slideUp 0.5s ease forwards", opacity: 0 }}>
              ...
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, fontWeight: 800, marginBottom: 8 }}>{saved.days} Tage · {savedGoal?.emoji} {savedGoal?.name}</div>
      ...
      {screen === "builder" && (
        <div style={{ paddingTop: 30 }}>
          <div style={{ textAlign: "center", marginBottom: 36 }}>
            ...
            <p style={{ fontSize: 15, color: "#78909C" }}>Schritt {builderStep} von 4</p>
          ...
          {builderStep === 1 && (
            <div style={{ animation: "slideUp 0.4s ease forwards", opacity: 0 }}>
              <div style={{ marginBottom: 28 }}>
                <label style={{ fontFamily: "'Space Mono', monospace", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#546E7A", display: "block", marginBottom: 10 }}>🏄 Dein Surfboard</label>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                  {BOARD_TYPES.map(b => (
                    <button key={b.id} onClick={() => setBoard(b.id)} style={{ background: board === b.id ? "linear-gradient(135deg, #5C6BC0, #7986CB)" : "rgba(255,255,255,0.8)", color: board === b.id ? "white" : "#263238", border: board === b.id ? "2px solid #5C6BC0" : "2px solid #E0E0E0", borderRadius: 14, padding: "14px 10px", fontSize: 13, fontWeight: 600, cursor: "pointer", textAlign: "center", transition: "all 0.2s ease" }}>
              ...
              <div style={{ marginBottom: 28 }}>
                <label style={{ fontFamily: "'Space Mono', monospace", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#546E7A", display: "block", marginBottom: 10 }}>🌿 Deine Erfahrung</label>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {EXPERIENCE_LEVELS.map(e => (
                    <button key={e.id} onClick={() => setExperience(e.id)} style={{ background: experience === e.id ? "linear-gradient(135deg, #66BB6A, #43A047)" : "rgba(255,255,255,0.8)", color: experience === e.id ? "white" : "#263238", border: experience === e.id ? "2px solid #43A047" : "2px solid #E0E0E0", borderRadius: 14, padding: "14px", fontSize: 14, fontWeight: 600, cursor: "pointer", textAlign: "left", transition: "all 0.2s ease" }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <button onClick={() => { setBoard(""); setExperience(""); setBuilderStep(2); }} style={{ background: "rgba(255,255,255,0.8)", color: "#78909C", border: "2px solid #E0E0E0", borderRadius: 14, padding: "16px 20px", fontSize: 13, cursor: "pointer" }}>Überspringen</button>
              </div>
            </div>
          )}
          ...
          {builderStep === 2 && (
            <div style={{ animation: "slideUp 0.4s ease forwards", opacity: 0 }}>
              <div style={{ marginBottom: 28 }}>
                <label style={{ fontFamily: "'Space Mono', monospace", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#546E7A", display: "block", marginBottom: 10 }}>📅 Wie viele Tage? <span style={{ fontSize: 28, fontWeight: 900, color: "#009688", fontFamily: "'Playfair Display', serif" }}>{days}</span></label>
                <input type="range" min="3" max="30" value={days} onChange={e => setDays(parseInt(e.target.value))} style={{ width: "100%", marginBottom: 8 }} />
                <div style={{ display: "flex", justifyContent: "space-between", fontFamily: "'Space Mono', monospace", fontSize: 11, color: "#90A4AE" }}>
                  <span>3 Tage</span>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setBuilderStep(1)} style={{ background: "rgba(255,255,255,0.8)", color: "#546E7A", border: "2px solid #E0E0E0", borderRadius: 14, padding: "16px 20px", fontSize: 14, cursor: "pointer" }}>← Zurück</button>
          ...
          {builderStep === 3 && (
            <div style={{ animation: "slideUp 0.4s ease forwards", opacity: 0 }}>
              <div style={{ marginBottom: 28 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {GOALS.map(g => (
                    <button key={g.id} onClick={() => setGoal(g.id)} style={{ background: goal === g.id ? "linear-gradient(135deg, #FF7043, #FFB74D)" : "rgba(255,255,255,0.8)", color: goal === g.id ? "white" : "#263238", border: goal === g.id ? "2px solid #FF7043" : "2px solid #E0E0E0", borderRadius: 14, padding: 16, fontSize: 14, fontWeight: 600, cursor: "pointer", textAlign: "left", transition: "all 0.2s ease" }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setBuilderStep(2)} style={{ background: "rgba(255,255,255,0.8)", color: "#546E7A", border: "2px solid #E0E0E0", borderRadius: 14, padding: "16px 20px", fontSize: 14, cursor: "pointer" }}>← Zurück</button>
          ...
          {builderStep === 4 && (
            <div style={{ animation: "slideUp 0.4s ease forwards", opacity: 0 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setBuilderStep(3)} style={{ background: "rgba(255,255,255,0.8)", color: "#546E7A", border: "2px solid #E0E0E0", borderRadius: 14, padding: "16px 20px", fontSize: 14, cursor: "pointer" }}>← Zurück</button>
        ...
        <div style={{ paddingTop: 24 }}>
          ...
          {program.spot?.tips && (
            <div style={{ background: "rgba(255,255,255,0.7)", borderRadius: 16, padding: "14px 18px", marginBottom: 20, border: "1px solid rgba(0,0,0,0.05)" }}>
              ...
              {program.spot.tips.map((tip, i) => (<div key={i} style={{ display: "flex", gap: 8, marginBottom: 4, fontSize: 13, color: "#37474F" }}><span style={{ color: "#5C6BC0" }}>•</span><span>{tip}</span></div>))}
            </div>
          )}
          <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap" }}>
            {[{ k: "all", l: "Alle", e: "📚" }, { k: "equipment", l: "Equipment", e: "🎒" }, { k: "warmup", l: "Warm-Up", e: "🔥" }, { k: "theory", l: "Theorie", e: "📖" }, { k: "practice", l: "Praxis", e: "🏄" }].map(f => (
              <button key={f.k} onClick={() => setFilter(f.k)} style={{ background: filter === f.k ? "#263238" : "rgba(255,255,255,0.8)", color: filter === f.k ? "white" : "#546E7A", border: "1px solid " + (filter === f.k ? "#263238" : "#E0E0E0"), borderRadius: 20, padding: "7px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>{f.e} {f.l}</button>
            ))}
          </div>
                ...
                <button onClick={() => setActiveDay(activeDay === dayData.day ? null : dayData.day)} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(255,255,255,0.6)", border: "1px solid rgba(0,0,0,0.06)", borderRadius: 14, padding: "12px 16px", cursor: "pointer" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div style={{ textAlign: "left" }}>
                      <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: 15, color: "#263238" }}>Tag {dayData.day}{dayData.day === 1 && dayData.lessons.some(l => l.type === "equipment") ? " · Equipment" : ""}</div>
